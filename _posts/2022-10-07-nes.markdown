...
- [Rotationally-symmetric distributions](rot-sym-dist)
	...
	- [Exponential natural evolution strategies (xNES)](#xnes)
...
{: #xnes}
Recall that the Multivariate Gaussian can be expressed in form of a radial distribution \eqref{eq:ep.1}.
...
\end{align}
where $\mathbf{s}\_{k:\lambda}$ denotes the $k$-th best sample in local coordinates.
