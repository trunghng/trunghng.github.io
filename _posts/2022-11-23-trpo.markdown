...
\end{equation}
Let us pay attention to our objective function, $L_{\theta_\text{old}}(\theta)$, for a while. By the definition of $L$, given in \eqref{eq:pi.5}, combined with using an [importance sampling estimator]({% post_url 2022-05-25-likelihood-ratio-pg-is %}), we can rewrite the objective function of \eqref{eq:ppo.1} as 
\begin{align}
...
## Sampled-based estimation
{: #sampled-bsd-est}
The objective and constraint functions of \eqref{eq:ppo.2} can be approximated using Monte Carlo simulation. Following are two possible sampling approach to construct the estimated objective and constraint functions.
...
### Vine
{: #vine}
This sampling approach follows this process
<ul id='number-list'>
	<li>
	...
	<li>
		Average over $s_n\sim\rho(\pi)$ to obtain an estimator for $L_{\theta_\text{old}}$, along with its gradient.
	</li>
</ul>
