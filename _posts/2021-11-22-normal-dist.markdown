...
Moreover, we have that the variation part which depends on $\mathbf{x}$ for any Gaussian $\mathbf{X}\sim\mathcal{N}(\mathbf{x}\vert\boldsymbol{\mu},\boldsymbol{\Sigma})$ can be written as
\begin{equation}
-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^\text{T}\boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})=-\frac{1}{2}\mathbf{x}^\text{T}\boldsymbol{\Sigma}^{-1}\mathbf{x}+\mathbf{x}^\text{T}\boldsymbol{\Sigma}^{-1}\boldsymbol{\mu}+c,
\end{equation}
...
\begin{equation}
\boldsymbol{\Sigma}\_{a\vert b}=\boldsymbol{\Lambda}\_{aa}^{-1},\tag{18}\label{18}
\end{equation}
and with the corresponding mean vector, denoted as $\boldsymbol{\mu}\_{a\vert b}$, given by
\begin{align}
\boldsymbol{\mu}\_{a\vert b}&=\boldsymbol{\Sigma}\_{a\vert b}\big(\boldsymbol{\Lambda}\_{aa}\boldsymbol{\mu}\_a-\boldsymbol{\Lambda}\_{ab}(\mathbf{x}\_b-\boldsymbol{\mu}\_b)\big) \\\\ &=\boldsymbol{\mu}\_a-\boldsymbol{\Lambda}\_{aa}^{-1}\boldsymbol{\Lambda}\_{ab}(\mathbf{x}\_b-\boldsymbol{\mu}\_b)\tag{19}\label{19}
\end{align}
...
\begin{align}
\left[\begin{matrix}\mathbf{A}&\mathbf{B} \\\\ \mathbf{C}&\mathbf{D}\end{matrix}\right]^{-1}=\left[\begin{matrix}\mathbf{M}&-\mathbf{M}\mathbf{B}\mathbf{D}^{-1} \\\\ -\mathbf{D}^{-1}\mathbf{C}\mathbf{M}&\mathbf{D}^{-1}+\mathbf{D}^{-1}\mathbf{C}\mathbf{M}\mathbf{B}\mathbf{D}^{-1}\end{matrix}\right],\tag{20}\label{20}
\end{align}
where we have defined
\begin{equation}
\mathbf{M}\doteq(\mathbf{A}-\mathbf{B}\mathbf{D}^{-1}\mathbf{C})^{-1},
\end{equation}
whose inverse $\mathbf{M}^{-1}$ is called the **Schur complement** of the matrix $\left[\begin{matrix}\mathbf{A}&\mathbf{B} \\\\ \mathbf{C}&\mathbf{D}\end{matrix}\right]^{-1}$. This identity can be proved by multiplying both sides of \eqref{20} with $\left[\begin{matrix}\mathbf{A}&\mathbf{B} \\\\ \mathbf{C}&\mathbf{D}\end{matrix}\right]$ to give
\begin{align}
...
Applying the identity \eqref{20} into the precision matrix $\boldsymbol{\Lambda}=\boldsymbol{\Sigma}^{-1}$ gives us
\begin{equation}
...
\end{align}
Substitute these results into \eqref{18} and \eqref{19}, we have the mean and the covariance matrix of the Gaussian conditional distribution $p(\mathbf{x}\_a\vert\mathbf{x}\_b)$ can be rewritten as
\begin{align}
