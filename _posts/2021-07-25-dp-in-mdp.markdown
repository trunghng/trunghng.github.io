...
\end{align}
for all $s\in\mathcal{S}$. Thanks to [Banach's fixed points theorem]({% post_url 2021-07-10-optimal-policy-existence %}#banach-fixed-pts) and as we have mentioned in that note, we have that the sequence $\\{v_k\\}\to v_\pi$ as $k\to\infty$. This algorithm is called **iterative policy evaluation**.  
...
\begin{align}
\pi'(s)&\doteq\underset{a}{\text{argmax}}\,q_\pi(s,a) \\\\ &=\underset{a}{\text{argmax}}\,\mathbb{E}\left[R_{t+1}+\gamma v_\pi(S_{t+1})|S_t=s,A_t=a\right]\tag{4}\label{4} \\\\ &=\underset{a}{\text{argmax}}\,\sum_{s',r}p(s',r|s,a)\left[r+\gamma v_\pi(s')\right]
\end{align}
