...
4. *Other Null Sets*: The length of a point is 0. The area of a line, or a curve is 0. The volumn of a plane or a surface is 0.
...
#### Definition  
Given an open set $S=\sum_k(a_k,b_k)$ cointaining disjoint invervals, the **Lebesgue measure** is defined by:
\begin{equation}
...
\end{equation}
Consider the infiniteesimal $\delta_{\\{\alpha\\}}(dx)$ as $x$ ranges over $\mathbb{R}$. If $x\neq\alpha$, then the infinitesimal interval $[x,x+dx[$ does not contain $\alpha$, so
\begin{equation}
...
- If $f:\mathcal{X}\to\mathbb{R}$ is a general function, we can dfeine its *positive part* $f^+$ and its *negative part* $f^-$ by
\begin{align}
...
\end{align}
- The measure $\mu$ counts how mnay elements of $$\\{0,1\\}$$ are in its argument. Thus, the counting measure of the integers can be re-expressed as
\begin{equation}
\delta_\mathbb{Z}=\sum_{i=-\infty}^{\infty}\delta_{\\{i\\}}
\end{equation}
- By combining the operations of summation and multiplication, we can write the Geometric measure in the above example as $$\sum_{i=0}^{\infty}r^i\delta_{\\{i\\}}$$
...
4. If there exists a countable set $\mathcal{S}$ such taht $\mu(\mathcal{X}-\mathcal{S})=0$, we say that $\mu$ is a *discrete measure*. Equivalently, $\mu$ has a density w.r.t *counting measure* on $\mathcal{S}$.
...
#### References
1. Litterally, this post is mainly written from a source that I've lost the reference :(. Hope that I can update this line soon.
