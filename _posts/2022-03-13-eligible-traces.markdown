...
\begin{align}
\hat{v}(S_t,\mathbf{w}\_t)&=\mathbf{w}\_t^T\mathbf{x}\_t; \\\\ \nabla_\mathbf{w}\hat{v}(S_t,\mathbf{w}\_t)&=\mathbf{x}\_t,
\end{align}
where $\mathbf{x}\_t=\mathbf{x}(S_t)$ as usual.

We begin by rewriting \eqref{4}, as
\begin{align}
\mathbf{w}\_{t+1}^h&\doteq\mathbf{w}\_t^h+\alpha\left[G_{t:h}^\lambda-\hat{v}(S_t,\mathbf{w}\_t^h)\right]\nabla_\mathbf{w}\hat{v}(S_t,\mathbf{w}\_t^h) \\\\ &=\mathbf{w}\_t^h+\alpha\left[G_{t:h}^\lambda-\left(\mathbf{w}\_t^h\right)^T\mathbf{x}\_t\right]\mathbf{x}\_t \\\\ &=\left(\mathbf{I}-\alpha\mathbf{x}\_t\mathbf{x}\_t^\T\right)\mathbf{w}\_t^h+\alpha\mathbf{x}\_t G_{t:h}^\lambda,
\end{align}
where $\mathbf{I}$ is the identity matrix. With this equation, consider $\mathbf{w}\_t^h$ in the cases of $t=1$ and $t=2$, we have:
\begin{align}
\mathbf{w}\_1^h&=\left(\mathbf{I}-\alpha\mathbf{x}\_0\mathbf{x}\_0^T\right)\mathbf{w}\_0^h+\alpha\mathbf{x}\_0 G_{0:h}^\lambda \\\\ &=\left(\mathbf{I}-\alpha\mathbf{x}\_0\mathbf{x}\_0^T\right)\mathbf{w}\_{init}+\alpha\mathbf{x}\_0 G_{0:h}^\lambda, \\\\ \mathbf{w}\_2^h&=\left(\mathbf{I}-\alpha\mathbf{x}\_1\mathbf{x}\_1^T\right)\mathbf{w}\_1^h+\alpha\mathbf{x}\_1 G_{1:h}^\lambda \\\\ &=\left(\mathbf{I}-\alpha\mathbf{x}\_1\mathbf{x}\_1^T\right)\left(\mathbf{I}-\alpha\mathbf{x}\_0\mathbf{x}\_0^T\right)\mathbf{w}\_{init}+\alpha\left(\mathbf{I}-\alpha\mathbf{x}\_1\mathbf{x}\_1^T\right)\mathbf{x}\_0 G_{0:h}^\lambda+\alpha\mathbf{x}\_1 G_{1:h}^\lambda
\end{align}
...
\begin{equation}
\mathbf{A}\_i^j\doteq\left(\mathbf{I}-\alpha\mathbf{x}\_j\mathbf{x}\_j^T\right)\left(\mathbf{I}-\alpha\mathbf{x}\_{j-1}\mathbf{x}\_{j-1}^T\right)\dots\left(\mathbf{I}-\alpha\mathbf{x}\_i\mathbf{x}\_i^T\right),\hspace{1cm}j\geq i,
\end{equation}
...
\begin{align}
\delta_t&\doteq\delta_t'-\mathbf{w}\_t^T\mathbf{x}\_t+\mathbf{w}\_{t-1}^T\mathbf{x}\_t \\\\ &=R_{t+1}+\gamma\mathbf{w}\_t^T\mathbf{x}\_{t+1}-\mathbf{w}\_t^T\mathbf{x}\_t,
\end{align}
...
\begin{align}
\mathbf{z}\_t&=\sum_{i=0}^{t}\mathbf{A}\_{i+1}^t\mathbf{x}\_i(\gamma\lambda)^{t-i} \\\\ &=\sum_{i=0}^{t-1}\mathbf{A}\_{i+1}^t\mathbf{x}\_i(\gamma\lambda)^{t-i}+\mathbf{x}\_t \\\\ &=\left(\mathbf{I}-\alpha\mathbf{x}\_t\mathbf{x}\_t^T\right)\gamma\lambda\sum_{i=0}^{t-1}\mathbf{A}\_{i+1}^{t-1}\mathbf{x}\_i(\gamma\lambda)^{t-i-1}+\mathbf{x}\_t \\\\ &=\left(\mathbf{I}-\alpha\mathbf{x}\_t\mathbf{x}\_t^T\right)\gamma\lambda\mathbf{z}\_{t-1}+\mathbf{x}\_t \\\\ &=\gamma\lambda\mathbf{z}\_{t-1}+\left(1-\alpha\gamma\lambda\left(\mathbf{z}\_{t-1}^T\mathbf{x}\_t\right)\right)\mathbf{x}\_t\tag{9}\label{9}
\end{align}
Equations \eqref{8} and \eqref{9} form the update of the **true online TD($\lambda$)** algorithm:
\begin{equation}
\mathbf{w}\_{t+1}\doteq\mathbf{w}\_t+\alpha\delta_t\mathbf{z}\_t+\alpha\left(\mathbf{w}\_t^T\mathbf{x}\_t-\mathbf{w}\_{t-1}^T\mathbf{x}\_t\right)\left(\mathbf{z}t\_t-\mathbf{x}\_t\right),
\end{equation}
where
\begin{align}
\mathbf{z}\_t&\doteq\gamma\lambda\mathbf{z}\_{t-1}+\left(1-\alpha\gamma\lambda\left(\mathbf{z}\_{t-1}^T\mathbf{x}\_t\right)\right)\mathbf{x}\_t,\tag{10}\label{10} \\\\ \delta_t&\doteq R_{t+1}+\gamma\mathbf{w}\_t^T\mathbf{x}\_{t+1}-\mathbf{w}\_t^T\mathbf{x}\_t
\end{align}
...
\begin{equation}
\mathbf{w}\_{k+1}^t=\mathbf{w}\_k+\eta_k\left(Y_k^t-\mathbf{x}\_k^T\mathbf{w}\_k^t\right)\mathbf{x}\_k+\mathbf{u}\_k,\hspace{1cm}0\leq k\lt t,
\end{equation}
...
\begin{align}
\mathbf{w}\_{t+1}&=\mathbf{w}\_t+(Y_t^{t+1}-Y_t^t)\mathbf{z}\_t+\eta_t(Y_t^t-\mathbf{x}\_t^T\mathbf{w}\_t)\mathbf{x}\_t+\mathbf{u}\_t, \\\\ \mathbf{z}\_t&=c_{t-1}\mathbf{z}\_{t-1}+\eta_t\left(1-c_{t-1}\mathbf{x}\_t^T\mathbf{z}\_{t-1}\right)\mathbf{x}\_t,\hspace{1cm}t\gt 0
\end{align}*

**Proof**  
Let $\mathbf{F}\_t\doteq\mathbf{I}-\eta_t\mathbf{x}\_t\mathbf{x}\_t^T$ be the *fading matrix* such that $\mathbf{w}\_{t+1}=\mathbf{F}\_k\mathbf{w}\_k^t+\eta_k Y_k^t\mathbf{x}\_k$. For each step $t$, we have:
\begin{align}
\mathbf{w}\_{t+1}^{t+1}-\mathbf{w}\_t^t&=\mathbf{F}\_t\mathbf{w}\_t^{t+1}-\mathbf{w}\_t^t+\eta_t Y_t^{t+1}\mathbf{x}\_t+\mathbf{u}\_t \\\\ &=\mathbf{F}\_t(\mathbf{w}\_t^{t+1}-\mathbf{w}\_t^t)+\eta_t Y_t^{t+1}\mathbf{x}\_t+(\mathbf{F}\_t-\mathbf{I})\mathbf{w}\_t^t+\mathbf{u}\_t \\\\ &=\mathbf{F}\_t(\mathbf{w}\_t^{t+1}-\mathbf{w}\_t^t)+\eta_t Y_t^{t+1}\mathbf{x}\_t-\eta_t\mathbf{x}\_t\mathbf{x}\_t^T\mathbf{w}\_t^t+\mathbf{u}\_t \\\\ &=\mathbf{F}\_t(\mathbf{w}\_t^{t+1}-\mathbf{w}\_t^t)+\eta_t(Y_t^{t+1}-\mathbf{x}\_t^T\mathbf{w}\_t^t)\mathbf{x}\_t+\mathbf{u}\_t\tag{12}\label{12}
\end{align}
We also have that:
\begin{align}
...
We begin with the new definition of $\lambda$-return, which is achieved by generalizing the $\lambda$-return \eqref{15} with the idea of [control variates on $n$-step off-policy return]({% post_url 2022-01-31-td-learning %}#n-step-return-control-variate-state-value):
\begin{equation}
...
For action-value function, we generalize the definition of the $\lambda$-return \eqref{16} of Expected Sarsa with the idea of [control variate]({% post_url 2022-01-31-td-learning %}#n-step-return-control-variate-action-value):
\begin{align}
...
As usual, we begin with establishing the $\lambda$-return by generalizing the $\lambda$-return of Expected Sarsa \eqref{16} with the [$n$-step Tree-backup return]({% post_url 2022-01-31-td-learning %}#n-step-tree-backup-return):
\begin{align}
...
Consider using linear function approximation, or in particular, we are trying to approximate $v(s)$ by $v_\mathbf{w}(s)=\mathbf{w}^T\mathbf{x}(s)$. Our objective is to find the fixed point which satisfies:
\begin{equation}
...
\end{equation}
where $\mathbf{D}$ denotes $\vert\mathcal{S}\vert\times\vert\mathcal{S}\vert$ diagonal matrix whose diagonal elements are $\mu(s)$, and $\mathbf{X}$ denotes the $\vert\mathcal{S}\vert\times d$ matrix whose rows are the feature vectors $\mathbf{x}(s)^T$, one for each state $s$.

With linear function approximation, we can rewrite the $\lambda$-return \eqref{22} as:
\begin{equation}
G_t^{\lambda}(\mathbf{w})\doteq R_{t+1}+\gamma_{t+1}\Big[(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1}+\lambda_{t+1}G_{t+1}^{\lambda}(\mathbf{w})\Big]\tag{24}\label{24}
\end{equation}
Let
\begin{equation}
\delta_t^\lambda(\mathbf{w})\doteq G_t^\lambda(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_t,
\end{equation}
and
\begin{equation}
...
\begin{align}
\overline{\text{PBE}}(\mathbf{w})&=\Big\Vert v_\mathbf{w}-\Pi T_\pi^\lambda v_\mathbf{w}\Big\Vert_\mu^2 \\\\ &=\Big\Vert\Pi(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w})\Big\Vert_\mu^2 \\\\ &=\Big(\Pi\left(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w}\right)\Big)^T\mathbf{D}\Big(\Pi\left(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w}\right)\Big) \\\\ &=\left(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w}\right)^T\Pi^T\mathbf{D}\Pi\left(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w}\right) \\\\ &=\left(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w}\right)^T\mathbf{D}^T\mathbf{X}\left(\mathbf{X}^T\mathbf{D}\mathbf{X}\right)^{-1}\mathbf{D}\left(v_\mathbf{w}-T_\pi^\lambda v_\mathbf{w}\right) \\\\ &=\Big(\mathbf{X}^T\mathbf{D}\left(T_\pi^\lambda v_\mathbf{w}-\mathbf{w}\right)\Big)^T\left(\mathbf{X}^T\mathbf{D}\mathbf{X}\right)^{-1}\mathbf{X}^T\mathbf{D}\left(T_\pi^\lambda v_\mathbf{w}-v_\mathbf{w}\right)\tag{25}\label{25}
\end{align}

From the definition of $T_\pi^\lambda$ and $\delta_t^\lambda$, we have:
\begin{align}
(T_\pi^\lambda v_\mathbf{w}-v_\mathbf{v})(s)&=\mathbb{E}\Big[G_t^\lambda(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_t\big|S_t=s,\pi\Big] \\\\ &=\mathbb{E}\Big[\delta_t^\lambda(\mathbf{w})\big|S_t=s,\pi\Big]\tag{26}\label{26}
\end{align}
Therefore,
\begin{align}
\mathbf{X}^T\mathbf{D}\left(T_\pi^\lambda v_\mathbf{w}-v_\mathbf{w}\right)&=\sum_s\mu(s)\Big[\left(T_\pi^\lambda v_\mathbf{w}-v_\mathbf{w}\right)(s)\Big]\mathbf{x}(s) \\\\ &=\sum_s\mu(s)\mathbb{E}\Big[\delta_t^\lambda(\mathbf{w})|S_t=s,\pi\Big]\mathbf{x}(s) \\\\ &=\mathcal{P}\_\mu^\pi\delta_t^\lambda(\mathbf{w})\mathbf{x}\_t\tag{27}\label{27}
\end{align}
Moreover, we also have:
\begin{equation}
\mathbf{X}^T\mathbf{D}\mathbf{X}=\sum_s\mu(s)\mathbf{x}(s)\mathbf{x}(s)^T=\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]\tag{28}\label{28}
\end{equation}
Substitute \eqref{26}, \eqref{27} and \eqref{28} back to the \eqref{25}, we have:
\begin{equation}
\overline{\text{PBE}}(\mathbf{w})=\Big(\mathcal{P}\_\mu^\pi\delta_t^\lambda(\mathbf{w})\mathbf{x}\_t\Big)^T\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\Big(\mathcal{P}\_\mu^\pi\delta_t^\lambda(\mathbf{w})\mathbf{x}\_t\Big)\tag{29}\label{29}
\end{equation}
...
\begin{equation}
G_t^{\lambda\rho}(\mathbf{w})=\rho_t\left(R_{t+1}+\gamma_{t+1}\left[(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1}+\lambda_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})\right]\right),
\end{equation}
where the single-step importance sampling ratio $\rho_t$ is defined as usual:
\begin{equation}
\rho_t\doteq\frac{\pi(A_t|S_t)}{b(A_t|S_t)}
...
\begin{equation}
\delta_t^{\lambda\rho}(\mathbf{w})\doteq G_t^{\lambda\rho}(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_t
\end{equation}
With this definition of the $\lambda$-return, we have:
\begin{align}
\mathbb{E}\Big[G_t^{\lambda\rho}(\mathbf{w})\big|S_t=s\Big]&=\mathbb{E}\Big[\rho_t\big(R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1}\big)+\rho_t\gamma_{t+1}\lambda_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_t=s\Big] \\\\ &=\mathbb{E}\Big[\rho_t\big(R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1}\big)\big|S_t=s\Big] \\\\ &\hspace{2cm}+\rho_t\gamma_{t+1}\lambda_{t+1}\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_t=s\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1}\big|S_t=s,\pi\Big] \\\\ &\hspace{2cm}+\sum_{a,s'}p(s'|s,a)b(a|s)\frac{\pi(a|s)}{b(a|s)}\gamma_{t+1}\lambda_{t+1}\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_{t+1}=s'\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1}\big|S_t=s,\pi\Big] \\\\ &\hspace{2cm}+\sum_{a,s'}p(s'|s,a)\pi(a|s)\gamma_{t+1}\lambda_{t+1}\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_{t+1}=s'\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\mathbf{x}\_{t+1} \\\\ &\hspace{2cm}+\gamma_{t+1}\lambda_{t+1}\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_{t+1}=s'\Big]\big|S_t=s,\pi\Big],
\end{align}
...
\begin{equation}
\overline{\text{PBE}}(\mathbf{w})=\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]^T\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]\tag{31}\label{31}
\end{equation}
...
\begin{align}
\frac{1}{2}\nabla_\mathbf{w}\overline{\text{PBE}}(\mathbf{w})&=-\frac{1}{2}\nabla_\mathbf{w}\Bigg(\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]^T\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]\Bigg) \\\\ &=\nabla_\mathbf{w}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\big(\gamma_{t+1}\mathbf{x}\_{t+1}-\mathbf{x}\_t\big)\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\mathbf{x}\_{t+1}\mathbf{z}\_t^T-\mathbf{x}\_t\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\mathbf{x}\_{t+1}\mathbf{z}\_t^T-\mathbf{x}\_t\rho_t\big(\mathbf{x}\_t+\gamma_t\lambda_t\mathbf{z}\_{t-1}\big)^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\mathbf{x}\_{t+1}\mathbf{z}\_t^T-\big(\mathbf{x}\_t\rho_t\mathbf{x}\_t^T+\mathbf{x}\_t\rho_t\gamma_t\lambda_t\mathbf{z}\_{t-1}^T\big)\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\mathbf{x}\_{t+1}\mathbf{z}\_t^T-\big(\mathbf{x}\_t\mathbf{x}\_t^T+\mathbf{x}\_{t+1}\gamma_{t+1}\lambda_{t+1}\mathbf{z}\_t^T\big)\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T-\gamma_{t+1}(1-\lambda_{t+1})\mathbf{x}\_{t+1}\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]-\mathbb{E}\Big[\gamma_{t+1}(1-\lambda_{t+1})\mathbf{x}\_{t+1}\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]-\mathbb{E}\Big[\gamma_{t+1}(1-\lambda_{t+1})\mathbf{x}\_{t+1}\mathbf{z}\_t^T\Big]\mathbf{v}(\mathbf{w}),\tag{32}\label{32}
\end{align}
where in the seventh step, we have used shifting indices trick and the identities:
\begin{align}
\mathbb{E}\Big[\mathbf{x}\_t\rho_t\mathbf{x}\_t^T\Big]&=\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big], \\\\ \mathbb{E}\Big[\mathbf{x}\_{t+1}\rho_t\gamma_t\lambda_t\mathbf{z}\_t^T\Big]&=\mathbb{E}\Big[\mathbf{x}\_{t+1}\gamma_t\lambda_t\mathbf{z}\_t^T\Big]
\end{align}
and where in the final step, we define:
\begin{equation}
\mathbf{v}(\mathbf{w})\doteq\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]
\end{equation}
By direct sampling from \eqref{32} and following TDC derivation steps we obtain the **GTD($\lambda$)** algorithm:
\begin{equation}
\mathbf{w}\_{t+1}\doteq\mathbf{w}\_t+\alpha\delta_t^s\mathbf{z}\_t-\alpha\gamma_{t+1}(1-\lambda_{t+1})(\mathbf{z}\_t^T\mathbf{v}\_t)\mathbf{x}\_{t+1},
\end{equation}
where
...
\begin{align}
\delta_t^s&\doteq R_{t+1}+\gamma_{t+1}\mathbf{w}\_t^T\mathbf{x}\_{t+1}-\mathbf{w}\_t^T\mathbf{x}\_t, \\\\ \mathbf{z}\_t&\doteq\rho_t(\gamma_t\lambda_t\mathbf{z}\_{t-1}+\mathbf{x}\_t), \\\\ \mathbf{v}\_{t+1}&\doteq\mathbf{v}\_t+\beta\delta_t^s\mathbf{z}\_t-\beta(\mathbf{v}\_t^T\mathbf{x}\_t)\mathbf{x}\_t
\end{align}

### GQ($\lambda$)
{: #gq-lambda}
**GQ($\lambda$)** is another eligible trace version of a Gradient-TD method but with action values. Its goal is to learn a parameter $\mathbf{w}\_t$ such that $\hat{q}(s,a,\mathbf{w}\_t)\doteq\mathbf{w}\_t^T\mathbf{x}(s,a)\approx q_\pi(s,a)$ from data given by following a behavior policy $b$.
...
\end{align}
Analogous to the state value functions, with linear function approximation (i.e., we are trying to estimate $q(s,a)$ by $q_\mathbf{w}(s,a)=\mathbf{w}^T\mathbf{x}(s,a)$), our objective is to find the fixed point $q_\mathbf{w}$ such that:
\begin{equation}
...
\begin{align}
\overline{\text{PBE}}(\mathbf{w})&=\left\Vert q_\mathbf{w}-\Pi T_\pi^\lambda q_\mathbf{w}\right\Vert_\mu^2 \\\\ &=\Big(\mathcal{P}\_\mu^\pi\delta_t^\lambda(\mathbf{w})\mathbf{x}\_t\Big)^T\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\Big(\mathcal{P}\_\mu^\pi\delta_t^\lambda(\mathbf{w})\mathbf{x}\_t\Big),\tag{34}\label{34}
\end{align}
...
\begin{equation}
G_t^{\lambda\rho}(\mathbf{w})\doteq R_{t+1}+\gamma_{t+1}\Big[(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}+\lambda_{t+1}\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})\Big],\tag{35}\label{35}
\end{equation}
...
Let
\begin{equation}
\delta_t^{\lambda\rho}(\mathbf{w})\doteq G_t^{\lambda\rho}(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_t\tag{36}\label{36}
\end{equation}
With the definition of the $\lambda$-return \eqref{35}, we have that:
\begin{align}
\mathbb{E}\Big[G_t^{\lambda\rho}(\mathbf{w})\big|S_t=s,A_t=s\Big]&=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}\Big((1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1} \\\\ &\hspace{1cm}+\lambda_{t+1}\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})\Big)\big|S_t=s,A_t=a\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}\big|S_t=s,A_t=a,\pi\Big] \\\\ &\hspace{1cm}+\gamma_{t+1}\lambda_{t+1}\mathbb{E}\Big[\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_t=s,A_t=a\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}\big|S_t=s,A_t=a,\pi\Big] \\\\ &\hspace{1cm}+\sum_{s'}p(s'|s,a)\sum_{a'}b(a'|s')\frac{\pi(a'|s')}{b(a'|s')}\gamma_{t+1}\lambda_{t+1} \\\\ &\hspace{1cm}\times\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_{t+1}=s',A_{t+1}=a'\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}\big|S_t=s,A_t=a,\pi\Big] \\\\ &\hspace{1cm}+\sum_{s',a'}p(s'|s,a)\pi(a'|s')\gamma_{t+1}\lambda_{t+1}\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_{t+1}=s',A_{t+1}=a'\Big] \\\\ &=\mathbb{E}\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1} \\\\ &\hspace{1cm}+\gamma_{t+1}\lambda_{t_1}\mathbb{E}\Big[G_{t+1}^{\lambda\rho}(\mathbf{w})\big|S_{t+1}=s',A_{t+1}=a'\Big]\big|S_t=s,A_t=a,\pi\Big],
\end{align}
...
\begin{align}
\delta_t^{\lambda\rho}(\mathbf{w})&=G_t^{\lambda\rho}(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_t \\\\ &=R_{t+1}+\gamma_{t+1}\Big[(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}+\lambda_{t+1}\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})\Big]-\mathbf{w}^T\mathbf{x}\_t \\\\ &=\Big[R_{t+1}+\gamma_{t+1}(1-\lambda_{t+1})\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}\Big]+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_t \\\\ &=\Big(R_{t+1}+\gamma_{t+1}\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}-\mathbf{w}^T\mathbf{x}\_t\Big)-\gamma_{t+1}\lambda_{t+1}\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w}) \\\\ &=\delta_t(\mathbf{w})-\gamma_{t+1}\lambda_{t+1}\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}G_{t+1}^{\lambda\rho}(\mathbf{w}) \\\\ &\hspace{2cm}+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}\Big(\mathbf{w}^T\mathbf{x}\_{t+1}-\mathbf{w}^T\mathbf{x}\_{t+1}\Big) \\\\ &=\delta_t(\mathbf{w})+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}\Big(G_{t+1}^{\lambda\rho}(\mathbf{w})-\mathbf{w}^T\mathbf{x}\_{t+1}\Big)+\gamma_{t+1}\lambda_{t+1}\Big(\rho_{t+1}\mathbf{w}^T\mathbf{x}\_{t+1}-\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}\Big) \\\\ &=\delta_t(\mathbf{w})+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}\delta_{t+1}^{\lambda\rho}(\mathbf{w})+\gamma_{t+1}\lambda_{t+1}\mathbf{w}^T\big(\rho_{t+1}\mathbf{x}\_{t+1}-\bar{\mathbf{x}}\_{t+1}\big),
\end{align}
where in the fifth step, we define:
\begin{equation}
\delta_t(\mathbf{w})\doteq R_{t+1}+\lambda_{t+1}\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}-\mathbf{w}^T\mathbf{x}\_t\tag{38}\label{38}
\end{equation}
...
\begin{equation}
\overline{\text{PBE}}(\mathbf{w})=\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]^T\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]
\end{equation}
Following the derivation of GTD($\lambda$), we have:
\begin{align}
-\frac{1}{2}\nabla_\mathbf{w}\overline{\text{PBE}}(\mathbf{w})&=-\frac{1}{2}\nabla_\mathbf{w}\Bigg(\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]^T\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]\Bigg) \\\\ &=\nabla_\mathbf{w}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\big(\gamma_{t+1}\bar{\mathbf{x}}\_{t+1}-\mathbf{x}\_t\big)\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T-\mathbf{x}\_t\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T-\mathbf{x}\_t\Big(\mathbf{x}\_t+\gamma_t\lambda_t\rho_t\mathbf{z}\_{t-1}\Big)^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T-\Big(\mathbf{x}\_t\mathbf{x}\_t^T+\gamma_t\lambda_t\rho_t\mathbf{x}\_t\mathbf{z}\_{t-1}^T\Big)\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T-\Big(\mathbf{x}\_t\mathbf{x}\_t^T+\gamma_{t+1}\lambda_{t+1}\rho_{t+1}\mathbf{x}\_{t+1}\mathbf{z}\_t^T\Big)\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T-\Big(\mathbf{x}\_t\mathbf{x}\_t^T+\gamma_{t+1}\lambda_{t+1}\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T\Big)\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=-\mathbb{E}\Big[\gamma_{t+1}(1-\lambda_{t+1})\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T-\mathbf{x}\_t\mathbf{x}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]-\mathbb{E}\Big[\gamma_{t+1}(1-\lambda_{t+1})\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T\Big]\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big] \\\\ &=\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]-\mathbb{E}\Big[\gamma_{t+1}(1-\lambda_{t+1})\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T\Big]\mathbf{v}(\mathbf{w}),
\end{align}
where in the eighth step, we have used the identity:
\begin{equation}
\mathbb{E}\Big[\rho_{t+1}\mathbf{x}\_{t+1}\mathbf{z}\_t^T\Big]=\mathbb{E}\Big[\bar{\mathbf{x}}\_{t+1}\mathbf{z}\_t^T\Big],
\end{equation}
and where in the final step, we define:
\begin{equation}
\mathbf{v}(\mathbf{w})\doteq\mathbb{E}\Big[\mathbf{x}\_t\mathbf{x}\_t^T\Big]^{-1}\mathbb{E}\Big[\delta_t(\mathbf{w})\mathbf{z}\_t\Big]
...
\begin{equation}
\mathbf{w}\_{t+1}\doteq\mathbf{w}\_t+\alpha\delta_t^a\mathbf{z}\_t-\alpha\gamma_{t+1}(1-\lambda_{t+1})(\mathbf{z}\_t^T\mathbf{v}\_t)\bar{\mathbf{x}}\_{t+1},
\end{equation}
where
...
\begin{align}
\bar{\mathbf{x}}\_t&\doteq\sum_a\pi(a|S_t)\mathbf{x}(S_t,a), \\\\ \delta_t^a&\doteq R_{t+1}+\lambda_{t+1}\mathbf{w}^T\bar{\mathbf{x}}\_{t+1}-\mathbf{w}^T\mathbf{x}\_t, \\\\ \mathbf{z}\_t&\doteq\gamma_t\lambda_t\rho_t\mathbf{z}\_{t-1}+\mathbf{x}\_t, \\\\ \mathbf{v}\_{t+1}&\doteq\mathbf{v}\_t+\beta\delta_t^a\mathbf{z}\_t-\beta(\mathbf{v}\_t^T\mathbf{x}\_t)\mathbf{x}\_t
\end{align}
...
\begin{equation}
\mathbf{w}\_{t+1}\doteq\mathbf{w}\_t+\alpha\delta_t^a\mathbf{x}\_t+\alpha\gamma_{t+1}(\mathbf{z}\_t^T\mathbf{x}\_t)\mathbf{x}(S_{t+1},a_{t+1}^{\*}),
\end{equation}
where the eligible trace $\mathbf{z}\_t$, TD error $\delta_t^a$ and $a_{t+1}^{\*}$ are defined as:
\begin{align}
\mathbf{z}\_t&\doteq\mathbf{z}\_t+\beta\delta_t^a\mathbf{x}\_t-\beta(\mathbf{z}\_t^T\mathbf{x}\_t)\mathbf{x}\_t, \\\\ \delta_t^a&\doteq R_{t+1}+\gamma_{t+1}\max_a\Big(\mathbf{w}\_t^T\mathbf{x}(S_{t+1},a)\Big)-\mathbf{w}\_t^T\mathbf{x}\_t, \\\\ a_{t+1}^{\*}&\doteq\arg\max\_a\Big(\mathbf{w}\_t^T\mathbf{x}(S_{t+1},a)\Big),
\end{align}
...
\begin{align}
\mathbf{w}\_{t+1}&\doteq\mathbf{w}\_t+\alpha\delta_t^s\mathbf{z}\_t+\alpha\left(\left(\mathbf{z}\_t-\mathbf{z}\_t^b\right)^T\mathbf{v}\_t\right)\left(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\right), \\\\ \mathbf{v}\_{t+1}&\doteq\mathbf{v}\_t+\beta\delta_t^s\mathbf{z}\_t-\beta\left({\mathbf{z}\_t^b}^T\mathbf{v}\_t\right)\left(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\right), \\\\ \mathbf{z}\_t&\doteq\rho_t\left(\gamma_t\lambda_t\mathbf{z}\_{t-1}+\mathbf{x}\_t\right), \\\\ \mathbf{z}\_t^b&\doteq\gamma_t\lambda_t\mathbf{z}\_{t-1}^b+\mathbf{x}\_t,
\end{align}

### Emphatic TD($\lambda$)
{: #em-td-lambda}
**Emphatic TD($\lambda$) (ETD($\lambda$))** is the extension of the [one-step Emphatic-TD algorithm]({% post_url 2022-02-11-func-approx %}#em-td) to eligible traces. 

Emphatic TD($\lambda$) or ETD($\lambda$) is defined by:
\begin{align}
\mathbf{w}\_{t+1}&\doteq\mathbf{w}\_t+\alpha\delta_t\mathbf{z}\_t, \\\\ \delta_t&\doteq R_{t+1}+\gamma_{t+1}\mathbf{w}\_t^T\mathbf{x}\_{t+1}-\mathbf{w}\_t^T\mathbf{x}\_t, \\\\ \mathbf{z}\_t&\doteq\rho_t\left(\gamma_t\lambda_t\mathbf{z}\_{t-1}+M_t\mathbf{x}\_t\right), \\\\ M_t&\doteq\gamma_t i(S_t)+(1-\lambda_t)F_t, \\\\ F_t&\doteq\rho_{t-1}\gamma_t F_{t-1}+i(S_t),
\end{align}
where
...
\begin{align}
\mathbf{w}\_{t+1}&\doteq\mathbf{w}\_t+\alpha\left(R_{t+1}+\gamma_{t+1}\mathbf{w}\_t^T\mathbf{x}\_{t+1}-\mathbf{w}\_t^T\mathbf{x}\_t\right)\mathbf{z}\_t \\\\ &=\mathbf{w}\_t+\alpha\left(\mathbf{z}\_t R_{t+1}-\mathbf{z}\_t\left(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\right)^T\mathbf{w}\_t\right)\tag{40}\label{40}
\end{align}
Let $\mathbf{A}\_t\in\mathbb{R}^d\times\mathbb{R}^d$ be a matrix and $\mathbf{b}\_t\in\mathbb{R}^d$ be a vector such that:
\begin{align}
\mathbf{A}\_t&\doteq\mathbf{z}\_t\left(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\right)^T, \\\\ \mathbf{b}\_t&\doteq\mathbf{z}\_t R_{t+1}
\end{align}
...
\begin{align}
\mathbf{A}&=\lim_{t\to\infty}\mathbb{E}\left[\mathbf{A}\_t\right] \\\\ &=\lim_{t\to\infty}\mathbb{E}\_b\Big[\mathbf{z}\_t\big(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\big)^T\Big] \\\\ &=\sum_s\mu(s)\lim_{t\to\infty}\mathbb{E}\_b\Big[\mathbf{z}\_t\big(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\big)^T\big|S_t=s\Big] \\\\ &=\sum_s\mu(s)\lim_{t\to\infty}\mathbb{E}\_b\Big[\rho_t\big(\gamma_t\lambda_t\mathbf{z}\_{t-1}+M_t\mathbf{x}\_t\big)\big(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\big)^T\big|S_t=s\Big] \\\\ &=\sum_s\mu(s)\lim_{t\to\infty}\mathbb{E}\_b\Big[\gamma_t\lambda_t\mathbf{z}\_{t-1}+M_t\mathbf{x}\_t\big|S_t=s\Big]\mathbb{E}\_b\Big[\rho_t\big(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1}\big)^T\big|S_t=s\Big] \\\\ &=\sum_s\underbrace{\mu(s)\lim_{t\to\infty}\mathbb{E}\_b\Big[\gamma_t\lambda_t\mathbf{z}\_{t-1}+M_t\mathbf{x}\_t\big|S_t=s\Big]}\_{\mathbf{z}(s)}\mathbb{E}\_b\Big[\rho_k\big(\mathbf{x}\_k-\gamma_{k+1}\mathbf{x}\_{k+1}\big)^T\big|S_k=s\Big] \\\\ &=\sum_s\mathbf{z}(s)\mathbb{E}\_\pi\Big[\mathbf{x}\_k-\gamma_{k+1}\mathbf{x}\_{k+1}\big|S_k=s\Big] \\\\ &=\sum_s\mathbf{z}(s)\Big(\mathbf{x}\_t-\sum_{s'}\left[\mathbf{P}\_\pi\right]\_{ss'}\gamma(s')\mathbf{x}(s')\Big)^T \\\\ &=\mathbf{Z}\left(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma}\right)\mathbf{X},\tag{41}\label{41}
\end{align}
where
- in the fifth step, given $S_t=s$, $\mathbf{z}\_{t-1}$ and $M_t$ are independent of $\rho_t(\mathbf{x}\_t-\gamma_{t+1}\mathbf{x}\_{t+1})^T$;
...
- $\mathbf{Z}$ is a $\vert\mathcal{S}\vert\times d$ matrix, whose rows are $\mathbf{z}(s)$'s (i.e., $\mathbf{Z}^T\doteq\left[\mathbf{z}(s_1),\dots,\mathbf{z}(s_{\vert\mathcal{S}\vert})\right]$), with $\mathbf{z}(s)\in\mathbb{R}^d$ is a vector defined by[^1]:
\begin{align}
...
\begin{align}
\mathbf{Z}^T&=\mathbf{X}^T\mathbf{M}+\mathbf{Z}^T\mathbf{P}\_\pi\mathbf{\Gamma}\mathbf{\Lambda} \\\\ \Rightarrow\mathbf{Z}^T&=\mathbf{X}^T\mathbf{M}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma}\mathbf{\Lambda})^{-1}
\end{align}
Substitute this equation back to \eqref{41}, we obtain:
\begin{equation}
\mathbf{A}=\mathbf{X}^T\mathbf{M}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma}\mathbf{\Lambda})^{-1}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma})\mathbf{X}\tag{43}\label{43}
\end{equation}
Doing similar steps, we can also obtain the ETD($\lambda$)'s $\mathbf{b}$ vector:
\begin{equation}
\mathbf{b}=\mathbf{Z}\mathbf{r}\_\pi=\mathbf{X}^T\mathbf{M}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma}\mathbf{\Lambda})^{-1}\mathbf{r}\_\pi,
\end{equation}
...
\begin{equation}
\mathbf{X}^T\mathbf{M}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma}\mathbf{\Lambda})^{-1}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma})\mathbf{X}=\sum_{i=1}^{\vert\mathcal{S}\vert}\mathbf{x}\_i^T\mathbf{M}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma}\mathbf{\Lambda})^{-1}(\mathbf{I}-\mathbf{P}\_\pi\mathbf{\Gamma})\mathbf{x}\_i
\end{equation}
...
\end{equation}
In order to prove the positive definiteness of $\mathbf{M}(\mathbf{I}-\mathbf{P}\_\pi^\lambda)$, analogous to the [proof]({% post_url 2022-02-11-func-approx %}#td-fixed-pt-proof) of the convergence to TD fixed point of semi-gradient TD, we use two lemmas:
- **Lemma 1**: *Any matrix $\mathbf{A}$ is positive definite iff the symmetric matrix $\mathbf{S}=\mathbf{A}+\mathbf{A}^T$ is positive definite*.
...
\begin{align}
\mathbf{f}&=\mathbf{i}+\mathbf{\Gamma}\mathbf{P}\_\pi^T\mathbf{f} \\\\ &=\mathbf{i}+\mathbf{\Gamma}\mathbf{P}\_\pi^T\mathbf{i}+(\mathbf{\Gamma}\mathbf{P}\_\pi^T)^2\mathbf{i}+\dots \\\\ &=\left(\mathbf{I}-\mathbf{\Gamma}\mathbf{P}\_\pi^T\right)^{-1}
\end{align}
...
\begin{align}
\mathbf{m}&=\mathbf{\Lambda}\mathbf{i}+(\mathbf{I}-\mathbf{\Lambda})\mathbf{f} \\\\ &=\mathbf{\Lambda}\mathbf{i}+(\mathbf{I}-\mathbf{\Lambda})(\mathbf{I}-\mathbf{\Gamma}\mathbf{P}\_\pi^T)^{-1}\mathbf{i} \\\\ &=\Big[\mathbf{\Lambda}(\mathbf{I}-\mathbf{\Gamma}\mathbf{P}\_\pi^T)+(\mathbf{I}-\mathbf{\Lambda})\Big]\left(\mathbf{I}-\mathbf{\Gamma}\mathbf{P}\_\pi^T\right)\mathbf{i} \\\\ &=\Big(\mathbf{I}-\mathbf{\Lambda}\mathbf{\Gamma}\mathbf{P}\_\pi^T\Big)\Big(\mathbf{I}-\mathbf{\Gamma}\mathbf{P}\_\pi^T\Big)^{-1}\mathbf{i} \\\\ &=\Big(\mathbf{I}-{\mathbf{P}\_\pi^\lambda}^T\Big)^{-1}\mathbf{i}
\end{align}
...
\begin{align}
\mathbf{1}^T{M}(\mathbf{I}-\mathbf{P}\_\pi^\lambda)&=\mathbf{m}^T(\mathbf{I}-\mathbf{P}\_\pi^\lambda) \\\\ &=\mathbf{i}^T(\mathbf{I}-\mathbf{P}\_\pi^\lambda)^{-1}(\mathbf{I}-\mathbf{P}\_\pi^\lambda) \\\\ &=\mathbf{i}^T
\end{align}
