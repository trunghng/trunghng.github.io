...
In particular, during M-step, for each variable $X$, and for each $U\in\text{Pa}\_X^\mathcal{G}$, we try to solve the optimization problem
\begin{align}
...
\end{align}
We end up with this pseudocode for EM in Bayesian network with table-CPDs.
