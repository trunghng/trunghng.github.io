...
- The gradient MC algorithm in the previous section converges to the global optimum of the $\overline{\text{VE}}$ under linear function approximation if $\alpha$ is reduced over time according to the [usual conditions]({{< ref "td-learning#stochastic-approx-condition" >}}). In particular, it converges to the fixed point, called $\mathbf{w}\_{\text{MC}}$, with:
\begin{align}
...
- The semi-gradient TD algorithm also converges under linear approximation. 
	...
	\end{align}
	This quantity, $\mathbf{w}\_{\text{TD}}$, is called the **TD fixed point**. And in fact, linear semi-gradient TD(0) converges to this point[^1].
...
Based on the tabular [$n$-step TD]({{< ref "td-learning#n-step-td-update" >}}) we have defined before, applying the semi-gradient method, we have the function approximation version of its, called <span id='semi-grad-n-step-td-update'>**semi-gradient $\boldsymbol{n}$-step TD**</span>, can be defined as:
\begin{equation}
...
\end{equation}
where the $n$-step return is generalized from the [tabular version]({{< ref "td-learning#n-step-return" >}}):
\begin{equation}
...
Suppose each state $s$ corresponds to a vector of $d$ numbers, $\mathbf{s}=(s_1,\dots,s_d)^\text{T}$, with each $s_i\in[0,1]$. The $i$-th feature in the order-$n$ Fourier cosine basis can then be written as:
\begin{equation}
...
\end{equation}
The two approximations in \eqref{eq:lstd.4} and \eqref{eq:lstd.5} could be implemented incrementally using the same [technique]({{< ref "monte-carlo-in-rl#incremental-method" >}}) we used to apply earlier so that they can be done in constant time per step. Even so, the update for $\widehat{\mathbf{A}}\_t$ would have the computational complexity of $O(d^2)$, and so is its memory required to hold the $\widehat{\mathbf{A}}\_t$ matrix. 
...
There is also a differential form of the two [TD errors]({{< ref "td-learning#td_error" >}}):
\begin{equation}
...
For the semi-gradient version of [$n$-step tree-backup]({{< ref "td-learning#n-step-tree-backup" >}}), called **semi-gradient $\boldsymbol{n}$-step tree-backup**, the update rule is:
\begin{equation}
\mathbf{w}\_{t+n}\doteq\mathbf{w}\_{t+n-1}+\alpha\big[G_{t:t+n}-\hat{q}(S_t,A_t,\mathbf{w}\_{t+n-1})\big]\nabla_\mathbf{w}\hat{q}(S_t,A_t,\mathbf{w}\_{t+n-1}),
\end{equation}
where
...
With a given stored estimate $\mathbf{v}\_t$ approximating \eqref{eq:gt.3}, we can apply SGD update to the parameter $\mathbf{w}\_t$:
\begin{align}
...
## Footnotes
[^1]: <span id='td-fixed-pt-proof'>**Proof**</span>    
...
\end{equation\*}
be the *sample mean* of $X_1$ through $X_n$. As $n\to\infty$, the sample mean $\overline{X}_n$ converges to the true mean $\mu$, with probability $1$.
...
[^5]: A $n\times n$ matrix $A$ is called **positive definite** if and only if for any non-zero vector $\mathbf{x}\in\mathbb{R}^n$, we always have
\begin{equation\*}
